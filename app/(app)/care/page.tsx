...
export default async function CarePage({ searchParams }: { searchParams: Promise<{ sel?: string }> }) {
  ...
  const { data: itemsRaw } = await db.from("care_items").select("id, customer_id, subject, body, status, assignee, resolution, linked_order_id, created_at").order("created_at", { ascending: false });
  ...
  const selected = items.find((i) => i.id === sel) ?? items[0];
      ...
      <div className="work">
        ...
        <div className="w-main"><div className="card">
          ...
          <div className="card-b flush"><div className="twrap"><table className="tbl">
            ...
            <tbody>
              {items.map((it) => (
                <tr key={it.id} className={selected && it.id === selected.id ? "sel" : ""}>
                  <td><Link className="clink" href={`/care?sel=${it.id}`}>{it.subject}</Link></td>
                  <td className="muted">{emailById.get(it.customer_id) ?? "—"}</td>
        ...
        <div className="w-side">
          {selected ? (
            <div className="col-card">
              ...
              <div className="fieldrow"><span className="fk">Customer</span><span className="fv clink"><Link href={`/customers/${selected.customer_id}`}>{emailById.get(selected.customer_id) ?? "—"}</Link></span></div>
